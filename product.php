...
<?php
$catCategories = $pdo->query("select * from categories");
$rowsCat = $catCategories->fetchAll(PDO::FETCH_ASSOC);
foreach($rowsCat as $allCat){
	?>
	<button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".<?php echo $allCat['catId']?>">
						<?php echo $allCat['catName']?>
					</button>
	<?php
}
?>
			...
			<div class="row isotope-grid">
				...
				if(isset($_GET['catId'])){
					$cId = $_GET['catId'];
					$catQuery = $pdo ->prepare("select * from products where productCatId= :pid");
					$catQuery->bindParam("pid",$cId);
					$catQuery->execute();
					$catRow = $catQuery->fetchAll(PDO::FETCH_ASSOC);
					...
					<div class="block2">
						<div class="block2-pic hov-img0">
							<img src="<?php echo $proImageAddress.$catVal['productImage']?>" alt="IMG-PRODUCT">
